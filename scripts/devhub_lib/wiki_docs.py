...
from .io import find_first, find_first_token, now_iso, parse_json_output
...
def write_wiki_artifact(config: dict[str, Any], table: str, payload: dict[str, Any], *, base_record_id: str = "") -> dict[str, Any]:
    ...
    title = incremental_wiki_title(base_title, base_record_id=base_record_id, write_time=write_time)
    folder_name = spec["area_fallback"]
    wiki_path = f"Dev Knowledge Hub / 10 Projects / {project} / {folder_name} / {title}"
    body = wiki_writeback_body(table, payload, title=title, base_title=base_title, base_record_id=base_record_id, write_time=write_time)
...
def write_report_wiki_artifact(config: dict[str, Any], *, kind: str, project: str, body: str) -> dict[str, Any]:
    ...
    layout = wiki_layout(config, project)
    write_time = wiki_writeback_timestamp()
    title = f"{write_time} - Report: {project} {kind}"
