...
4. Use `--wiki` for durable retrospectives or long narratives worth reading later. This writes the Base row first, then creates a new timestamped Wiki page and indexes it in Base `Artifacts`.
...
Normal `record-*` commands write Base only. Use `--wiki` when the user expects a human-readable Wiki page for Bugfixes, AI Runs, Releases, Decisions, or Project Facts. Wiki pages are incremental by default: each write gets a fresh timestamped page instead of overwriting the previous page with the same title.
